...
- Little-endian ELF relocatable objects, an AVM-aware LLD port, and a final flat-image writer that creates the AVM header and tail locator.
...
6. Before editing, inspect `git status`, the current diff under `deps/llvm-project`, and untracked files. Extend existing AVM work; do not reapply or recreate changes already present.
7. Before the Clang phase, do not require `clang` to be in the default build. The default project list is currently `lld` only.
8. Do not silently change the ISA or ABI. When `docs/arch.md` is ambiguous or conflicts with LLVM requirements, record the issue in `docs/avm-llvm-open-issues.md`, choose the least invasive temporary implementation only when necessary, and add a test that makes the temporary behavior visible.
9. Treat `EM_AVM = 0x4156` and any AVM relocation numbers as private development assignments until documented otherwise. Keep them centralized and stable within this fork.
10. Avoid broad cleanup. Touch unrelated targets or target-independent LLVM code only when registration or a genuinely missing generic hook requires it.
11. Prefer small MIR, LLVM IR, MC, object, Clang, and LLD `lit` tests over large end-to-end fixtures until the final phases.
12. Preserve generated-file discipline: edit TableGen and source inputs, not generated build outputs.
...
- **Gate C — Minimal code generation:** an empty LLVM function lowers to valid AVM assembly and a `RET`.
- **Gate D — Core C subset:** integer arithmetic, branches, loads/stores, frames, and calls compile correctly.
- **Gate E — Harvard address spaces:** program pointers, `PB`, program loads, and indirect far calls work.
- **Gate F — Linkable Clang toolchain:** after explicitly enabling Clang, it emits AVM objects and LLD links them with AVM relocations.
- **Gate G — Runnable image:** the image writer creates a conforming header, payload layout, CRC, padding, and tail locator.
- **Gate H — Hardened backend:** conformance, diagnostics, profiling, and optimization tests are in place.
...
```text
You are implementing one narrowly scoped step of the AVM LLVM toolchain. Work from the repository root. Refer to `docs/arch.md` for all target information and treat it as the architectural source of truth. The LLVM fork is in `deps/llvm-project`; inspect that checked-in revision and nearby targets before choosing APIs or file structure. Inspect the existing AVM changes, `git status`, tracked diffs, and untracked files before editing; extend the current implementation and do not recreate work that is already present. Use the repository-root `CMakeLists.txt` as the normal build entry point and preserve command-line cache overrides. The initial minimal baseline leaves `LLVM_EXPERIMENTAL_TARGETS_TO_BUILD` empty; do not enable AVM before the plan step that creates the skeletal AVM target component libraries. Use the existing `build/` directory and its configured generator. Build only the Debug configuration: every `cmake --build` invocation must include `--config Debug`, every `ctest` invocation must include `-C Debug`, and generated tools must be invoked from the Debug output location. Assume all preceding steps in the implementation plan are complete. Do not implement later steps. Keep the tree buildable, add focused tests when appropriate, run them, and stop after reporting changed files, test commands, results, and any open issue.
...
7. Create or update `docs/avm-llvm-bringup.md` with the exact root configure/build/test commands, LLVM revision, build generator, enabled projects, discovered AVM files, and current pass/fail state.
8. Create or update `docs/avm-llvm-open-issues.md` with sections for current-scaffold blockers, ISA/ABI questions, LLVM integration questions, and deferred optimization work.
...
Required deliverables: `docs/avm-llvm-bringup.md`, `docs/avm-llvm-open-issues.md`, and a concise inventory of existing, missing, and untracked AVM integration files.
...
```text
You are implementing one narrowly scoped step of the AVM LLVM toolchain. Work from the repository root. Refer to `docs/arch.md` for all target information and treat it as the architectural source of truth. The LLVM fork is in `deps/llvm-project`; inspect that checked-in revision and nearby targets before choosing APIs or file structure. Inspect the existing AVM changes, `git status`, tracked diffs, and untracked files before editing; extend the current implementation and do not recreate work that is already present. Use the repository-root `CMakeLists.txt` as the normal build entry point and preserve command-line cache overrides. The initial minimal baseline leaves `LLVM_EXPERIMENTAL_TARGETS_TO_BUILD` empty; do not enable AVM before the plan step that creates the skeletal AVM target component libraries. Use the existing `build/` directory and its configured generator. Build only the Debug configuration: every `cmake --build` invocation must include `--config Debug`, every `ctest` invocation must include `-C Debug`, and generated tools must be invoked from the Debug output location. Assume Step 1 is complete. Do not implement instruction descriptions, MC encoding, GlobalISel, or full linker relocation semantics in this step. Keep the tree buildable, add focused tests, run them, and stop after reporting changed files, test commands, results, and any open issue.
...
3. Preserve `EM_AVM = 0x4156` as the current private development value unless Step 1 found a documented conflict. Centralize its status in `docs/avm-elf.md`; do not scatter numeric literals.
...
Required deliverables: Completed target-independent scaffold, `ELFRelocs/AVM.def`, minimal coherent LLD target handling, `docs/avm-elf.md`, and focused tests.
...
Required deliverables: MC asm backend, ELF object writer, fixup-to-existing-relocation mapping, any necessary updates to `docs/avm-elf.md`, and object-file tests.
...
## Step 11: Add the GlobalISel pipeline skeleton
...
## Step 12: Implement AVM GlobalISel legalization rules
...
## Step 13: Implement register-bank mapping and operand constraints
...
## Step 14: Implement ABI argument and return assignment in CallLowering
...
## Step 15: Select constants, copies, and basic register operations
...
Required deliverables: Initial instruction-selector logic, copy support, minimal return emission, and Gate C tests.
...
## Step 16: Select core ALU operations and model FLAGS explicitly
...
## Step 17: Lower conditional and unconditional branches
...
## Step 18: Lower data-space loads, stores, and address arithmetic
...
## Step 19: Eliminate frame indexes and select stack-relative accesses
...
## Step 20: Implement prologues, epilogues, callee saves, and stack diagnostics
...
## Step 21: Lower direct calls, returns, and tail calls
...
## Step 22: Implement the 24-bit program-pointer register representation
...
## Step 23: Lower PB state, program-space loads, and indirect control flow
...
## Step 24: Complete i8, i1, extension, and boolean lowering
...
## Step 25: Implement i32 multiword arithmetic, comparisons, shifts, and memory
...
## Step 26: Implement shifts, multiply lowering, and runtime libcalls
...
## Step 27: Implement variadic functions and remaining aggregate ABI cases
...
## Step 28: Implement global addresses, sections, constant pools, and constructor arrays
...
## Step 29: Implement switch lowering policy
...
## Step 30: Form postincrement memory operations before register allocation
...
## Step 31: Implement the post-register-allocation compact-encoding pass
...
## Step 32: Expand branch/call pseudos and emit relaxation metadata
...
## Step 33: Add target cost models, allocation preferences, and codegen tuning
...
## Step 34: Enable Clang in the root build and add AVM target data-model and ABI support
...
```text
You are implementing one narrowly scoped step of the AVM LLVM toolchain. Work from the repository root. Refer to `docs/arch.md` for all target information and treat it as the architectural source of truth. The LLVM fork is in `deps/llvm-project`; inspect that checked-in revision and nearby targets before choosing APIs or file structure. Inspect the existing AVM changes, `git status`, tracked diffs, and untracked files before editing; extend the current implementation and do not recreate work that is already present. The repository-root `CMakeLists.txt` currently enables only LLD by default. Reconfigure the existing `build/` tree in place with `-DLLVM_ENABLE_PROJECTS="clang;lld"`; do not create a parallel build directory and do not switch generators. Build only the Debug configuration: every `cmake --build` invocation must include `--config Debug`, every `ctest` invocation must include `-C Debug`, and generated tools must be invoked from the Debug output location. Do not add `FORCE` to cache variables, and do not change the root default merely to make this one step build. Assume all preceding steps are complete. Do not implement driver libraries, runtime helpers, or later linker work in this step. Keep the tree buildable, add focused tests, run them, and stop after reporting changed files, test commands, results, and any open issue.
...
Required work:
1. Reconfigure from the repository root with Clang and LLD enabled while preserving all existing generator, platform, toolchain, and AVM experimental-target options. Record the exact command in `docs/avm-llvm-bringup.md`.
...
## Step 35: Add Clang driver integration and the program-space source extension
...
## Step 36: Provide compiler runtime builtins and the freestanding startup ABI
...
## Step 37: Complete the existing AVM support in LLD's ELF linker
...
```text
You are implementing one narrowly scoped step of the AVM LLVM toolchain. Work from the repository root. Refer to `docs/arch.md` for all target information and treat it as the architectural source of truth. The LLVM fork is in `deps/llvm-project`; inspect that checked-in revision and nearby targets before choosing APIs or file structure. Inspect the existing AVM changes, `git status`, tracked diffs, and untracked files before editing; extend the current implementation and do not recreate work that is already present. Use the repository-root CMake configuration, which already enables LLD by default and already contains AVM LLD build/dispatch hooks from the current patch. Use the existing `build/` directory and its configured generator. Build only the Debug configuration: every `cmake --build` invocation must include `--config Debug`, every `ctest` invocation must include `-C Debug`, and generated tools must be invoked from the Debug output location. Assume all preceding steps are complete. Do not implement bank-aware placement, relaxation, veneers, or the flat-image writer in this step. Keep the tree buildable, add focused tests, run them, and stop after reporting changed files, test commands, results, and any open issue.
...
## Step 38: Implement bank constraints and initial bank-aware section placement
...
## Step 39: Implement linker relaxation and veneers
...
## Step 40: Implement and integrate the AVM flat-image writer

**Depends on:** Step 39

**Goal:** Convert the linked AVM ELF into the exact flat linked-image format required by the interpreter and emulator, and integrate the tool with the repository-root build without disturbing the focused LLVM+LLD defaults.

### Agent prompt

```text
You are implementing one narrowly scoped step of the AVM LLVM toolchain. Work from the repository root. Refer to `docs/arch.md` for all target information and treat it as the architectural source of truth. The LLVM fork is in `deps/llvm-project`; inspect that checked-in revision and nearby targets before choosing APIs or file structure. Inspect the existing AVM changes, `git status`, tracked diffs, and untracked files before editing; extend the current implementation and do not recreate work that is already present. Use the repository-root `CMakeLists.txt` as the normal build entry point. It enables LLVM tools and LLD by default; integrate the image tool so it can be built intentionally without forcing Clang or unrelated targets. Use the existing `build/` directory and its configured generator. Build only the Debug configuration: every `cmake --build` invocation must include `--config Debug`, every `ctest` invocation must include `-C Debug`, and generated tools must be invoked from the Debug output location. Assume all preceding steps are complete. Do not change the linked-image format or interpreter contract outside `docs/arch.md`. Keep the tree buildable, add focused tests, run them, and stop after reporting changed files, test commands, results, and any open issue.

Objective: Implement an AVM image writer/validator and make it available through the existing root build workflow.

Required work:
1. Choose a maintainable location for a dedicated AVM image tool. Prefer an LLVM-style tool under `deps/llvm-project/llvm/tools` if it can consume LLVM object libraries cleanly and be controlled by `LLVM_BUILD_TOOLS`; otherwise place it in a clearly separated project-owned tools directory and add it from the root CMake file without forcing unrelated LLVM projects.
2. Document and test the tool's explicit build target name. Preserve the root cache defaults and command-line override behavior; do not add `FORCE` or enable Clang merely for this tool.
3. Consume the linked AVM ELF and reserve the first 256 bytes for the header. Emit the contiguous `.data` initializer at logical/file offset `0x000100`, compute `programStart = align_up(0x100 + dataSize, 0x100)`, and fill alignment gaps with `0xFF`.
4. Lay out linked program-space sections at their linker-assigned logical addresses, preserving bank placements and allowing code/readonly-data interleaving.
5. Compute `dataSize`, `staticSize`, `saveSize`, and the 24-bit entry point. Enforce all size limits, section-space rules, and entry-point validity requirements from `docs/arch.md`.
6. Serialize header magic `41 56 4D 01`, `runtimeVersion`, fields, zero reserved bytes, and CRC-32/ISO-HDLC over bytes `0x00-0xFB`, storing the CRC little-endian at `0xFC`.
7. Choose `fileSize = align_up(endOfPayload + 8, 0x100)`, fill final padding with `0xFF`, and write the mandatory eight-byte tail with magic `41 56 54 01`, image page count, and zero reserved field.
8. Add validation and inspection modes for existing images, including header/tail decoding, CRC verification, page-count reconstruction, section/layout reporting, and development-placement checks.
9. Add golden binary tests, malformed-ELF tests, corruption tests, page-count tests, maximum static/save-size tests, reproducibility tests, and a build-system test that invokes the tool target from the root build.

Acceptance checks:
1. The output file offset equals logical program address for every stored byte.
2. The image length is a multiple of 256 and the tail page count exactly matches it.
3. Header CRC, reserved bytes, padding, and all architecture invariants validate.
4. The tool builds through a documented target in the root CMake workflow without changing the default enabled LLVM projects.

Required deliverables: Integrated AVM image writer/validator, root-build target and documentation, and golden/negative tests.

Before finishing, build the image-tool target through the repository-root configuration and run its focused tests plus the relevant LLD layout tests. Do not claim success if tests were not run; report the exact blocker instead. Do not edit generated build products. Do not make opportunistic changes outside this step.
```

---

## Step 41: Add debug information, assembler directives, and stack metadata support
...
## Step 42: Build an end-to-end conformance test matrix
...
## Step 43: Add negative testing, fuzzing, and diagnostics hardening
...
## Step 44: Add profiling instrumentation and tune AVM-specific policies
...
Required deliverables: Profiling/reporting support, benchmark corpus, tuned policies, and `docs/avm-isa-profiling-results.md`.
...
## Step 45: Finalize documentation, compatibility contracts, and release gates
...
| LLD source-list and target dispatch | Already started | Step 2 supplies a minimal coherent target; Step 37 completes relocation semantics |
...
| Clang | Not enabled by root default and not shown in diff | Step 34 explicitly enables and implements it |
| Flat image tool | Not shown in diff | Step 40 integrates it without forcing unrelated LLVM projects |

When the actual working tree differs from this snapshot, update `docs/avm-llvm-bringup.md` and follow the source tree rather than forcing it to match the old patch.
